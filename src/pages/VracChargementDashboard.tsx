...
const VracChargementDashboard: React.FC = () => {
    ...
    // Default to Today for Dashboard, will be customizable for History
    ...
    // Reset date filter when switching back to Dashboard
    useEffect(() => {
    ...
    const handleValidate = async (demandeId: string, tonnage: number, notes?: string): Promise<boolean> => {
        try {
            ...
            toast({
                ...
                description: `Tonnage enregistré avec succès`,
            });
    ...
    // Filter demandes locally for search/status
    ...
    // Stats based on filtered data (which is already date-filtered by backend)
    const stats = {
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
                <Tabs value={currentTab} onValueChange={(v) => setCurrentTab(v as 'dashboard' | 'history')} className="space-y-6">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        {/* Date Range Picker - Only visible in History, or read-only/indicator in Dashboard */}
                        {currentTab === 'history' ? (
                            <Popover>
                            ...
                            <div className="flex items-center gap-2 text-muted-foreground bg-muted/50 px-3 py-2 rounded-md border border-border/50 text-sm">
                                <CalendarIcon className="w-4 h-4" />
                                <span>Aujourd'hui ({format(new Date(), 'dd MMMM yyyy', { locale: fr })})</span>
                            </div>
                        )}
                    </div>
                    ...
                    <TabsContent value="dashboard" className="space-y-6">
                                ...
                                <CardContent className="p-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-muted-foreground">Camions chargés</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {['SIMAM', 'VIVO', 'TOTAL', 'PETRO IVOIRE'].map((clientName) => {
                                ...
                                const clientTonnage = clientDemandes.reduce((sum, d) => sum + (d.tonnage_charge || 0), 0);
