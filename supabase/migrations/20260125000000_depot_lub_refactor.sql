-- Migration: Refonte SIGMA vers Dépôt LUB
-- Description:
-- 1. Renommer 'sigma' en 'depot_lub' dans warehouse_type ENUM
-- 2. Renommer les tables sigma_stock et sigma_stock_history
-- 3. Renommer toutes les fonctions RPC
-- 4. Activer les mouvements miroirs pour depot_lub <-> bouteilles_neuves
-- 5. Améliorer le filtrage par date dans get_stock_movements_paginated
-- 6. Permettre l'édition de tous les champs dans update_stock_movement
...
-- =============================================
-- PHASE 3: Recréer les fonctions RPC avec nouveaux noms
-- =============================================

-- 3.1 get_depot_lub_stock (remplace get_sigma_stock)
...
-- 3.2 update_depot_lub_stock (remplace update_sigma_stock)
...
-- 3.3 decrement_depot_lub_stock (remplace decrement_sigma_stock)
...
-- 3.4 increment_depot_lub_stock (nouvelle fonction pour ajouter du stock)
...
-- 3.5 can_reduce_depot_lub_stock (remplace can_reduce_sigma_stock)
...
-- 3.6 update_depot_lub_stock_with_history (remplace update_sigma_stock_with_history)
...
BEGIN
  ...
  -- Insert or update depot_lub_stock
  INSERT INTO depot_lub_stock (client, bottle_type, current_stock)
...
-- 3.7 get_depot_lub_stock_history (remplace get_sigma_stock_history)
...
-- 3.8 update_depot_lub_stock_threshold (nouvelle fonction pour les seuils d'alerte)
...
-- =============================================
-- PHASE 4: Mettre à jour create_stock_movement avec miroirs depot_lub
-- =============================================
...
BEGIN
  ...
  -- For entries into Bouteilles Neuves FROM depot_lub, check and decrement depot_lub stock
  IF p_warehouse = 'bouteilles_neuves' AND p_movement_type = 'entree' AND p_source_warehouse = 'depot_lub' THEN
    -- Get current Depot LUB stock for detailed error message
  ...
  -- For exits from depot_lub to bouteilles_neuves, check depot_lub stock
  IF p_warehouse = 'depot_lub' AND p_movement_type = 'sortie' AND p_destination_warehouse = 'bouteilles_neuves' THEN
    ...
    -- Decrement depot_lub stock
    PERFORM decrement_depot_lub_stock(p_client, COALESCE(p_quantity_b6, 0), COALESCE(p_quantity_b12, 0));
  END IF;

  -- For exits from other warehouses (not depot_lub), check theoretical stock
  ...
  -- Create mirror movement for inter-warehouse transfers
  -- NOW INCLUDING depot_lub <-> bouteilles_neuves transfers
  IF p_movement_type = 'sortie' AND p_destination_warehouse IS NOT NULL THEN
    -- Mirror for depot_lub -> bouteilles_neuves
    ...
    -- Mirror for other inter-warehouse transfers (excluding depot_lub as destination)
  ...
  -- Create mirror for entry from depot_lub (reverse direction)
...
-- =============================================
-- PHASE 5: Améliorer get_stock_movements_paginated avec filtrage date avancé
-- =============================================

CREATE OR REPLACE FUNCTION get_stock_movements_paginated(
  p_warehouse warehouse_type,
  p_client stock_client_type,
  p_filter_type TEXT DEFAULT 'all',  -- 'all', 'year', 'month', 'range', 'day'
...
-- =============================================
-- PHASE 6: Améliorer update_stock_movement pour éditer tous les champs
-- =============================================
...
BEGIN
  ...
  IF v_linked_id IS NOT NULL THEN
    -- Determine the mirror movement type (opposite of main)
    UPDATE stock_movements SET
      movement_type = CASE
...
-- =============================================
-- PHASE 7: Supprimer les anciennes fonctions
-- =============================================
...
-- =============================================
-- PHASE 8: Mettre à jour les politiques RLS
-- =============================================
